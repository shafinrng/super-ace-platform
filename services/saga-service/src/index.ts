...
app.post("/saga/spin", async (req, res) => {
  try {
    const { playerId, sessionId, betAmount, currency, clientSeed, reelLengths, isFreeSpinMode, freeSpinMultiplier } = req.body;
    ...
    const saga = await executeSpinSaga({
      playerId, sessionId, betAmount, currency, clientSeed, reelLengths,
      isFreeSpinMode: !!isFreeSpinMode,
      freeSpinMultiplier: freeSpinMultiplier || 1,
    });
