...
$services = @(
    ...
    @{Name="payment-service"; Path="services/payment-service"; Port=3004; Env=@{}},
...
foreach ($svc in $services) {
    ...
    $envBlock = $svc.Env.GetEnumerator() | ForEach-Object { "$($_.Key)=$($_.Value)" }
    Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd `"C:\super-ace-platform\$($svc.Path)`"; $envBlock; npm run dev" -WindowStyle Minimized
    Start-Sleep -Milliseconds 500
}

Write-Host "`nAll services starting in minimized windows. Check port status in 10 seconds..." -ForegroundColor Green
...
foreach ($port in $ports) {
    ...
    if ($conn) { Write-Host "Port $port : RUNNING" -ForegroundColor Green }
